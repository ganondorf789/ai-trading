...
// 风控默认配置
const defaultRiskConfig: RiskControlConfig = {
...
export default function RiskControlTab({ onHasChanges }: RiskControlTabProps) {
  const [config, setConfig] = useState<RiskControlConfig>(defaultRiskConfig);
  const [originalConfig, setOriginalConfig] = useState<RiskControlConfig>(defaultRiskConfig);
  ...
  const loadConfig = useCallback(async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        const loadedConfig = { ...defaultRiskConfig, ...response.data };
        setConfig(loadedConfig);
        setOriginalConfig(loadedConfig);
      }
    } catch (error) {
      console.error("Failed to load risk control config:", error);
      addToast({ title: "加载风控配置失败", color: "danger" });
    } finally {
      setLoading(false);
    }
  }, []);
