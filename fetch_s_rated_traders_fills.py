    ...
    while current < end_dt:
        ...
        if hour_fills:
            if len(hour_fills) >= 2000:
                logger.warning(f"          {current.format('MM-DD HH:00')}: {len(hour_fills)} 条（达到上限，按分钟细分...）")
                minute_fills = fetch_fills_by_minutes(client, address, current, next_hour, db)
                all_fills.extend(minute_fills)
            else:
                all_fills.extend(hour_fills)
                if len(hour_fills) >= 500:
...
def fetch_fills_by_minutes(
    ...
    while current < end_dt:
        ...
        try:
            ...
            error_msg = f"获取分钟数据失败: {repr(e)}"
            logger.error(f"            ✗ {current.format('MM-DD HH:mm')}: {error_msg}")
            if db:
                db.record_fetch_fail(address, 'minute', error_msg, start_ms, end_ms)
            current = next_minute
            time.sleep(3.0)
            continue
        ...
        if minute_fills:
            ...
            if len(minute_fills) >= 2000:
                logger.error(f"            {current.format('MM-DD HH:mm')}: {len(minute_fills)} 条（单分钟达到2000条上限！无法进一步细分）")
            elif len(minute_fills) >= 100:
                logger.warning(f"            {current.format('MM-DD HH:mm')}: {len(minute_fills)} 条")
            else:
                logger.info(f"            {current.format('MM-DD HH:mm')}: {len(minute_fills)} 条")
...
def main():
    ...
    logger.info("等待 3 秒后开始（按 Ctrl+C 取消）")
    try:
        time.sleep(3)
