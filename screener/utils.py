...
def calculate_trade_type(dir_val: str, start_position: float) -> Optional[int]:
    ...
    Args:
        dir_val: 方向字符串，如 'Open Long', 'Close Short', 'Short > Long', 'Buy', 'Sell' 等
        start_position: 开始仓位（正数=多头，负数=空头）
    ...
    # Buy: 买入
        ...
        if start_pos < 0:
            return 6  # CLOSE_SHORT（平空）
        else:
            return 1 if start_pos == 0 else 2  # OPEN_LONG or ADD_LONG
    # Sell: 卖出
    elif dir_val == 'Sell':
        if start_pos > 0:
            return 3  # CLOSE_LONG（平多）
        else:
            return 4 if start_pos == 0 else 5  # OPEN_SHORT or ADD_SHORT
...
# ============================================================
# 仓位历史重建函数
# 用于从成交记录重建完整的仓位周期，以便更准确地计算胜率等指标
# ============================================================

# 交易类型常量（与 models.TradeType 保持一致）
_OPEN_LONG = 1
_ADD_LONG = 2
_CLOSE_LONG = 3
_OPEN_SHORT = 4
...
# 类型分组
_NEW_POSITION_TYPES = {_OPEN_LONG, _OPEN_SHORT}  # 新开仓
_ADD_POSITION_TYPES = {_ADD_LONG, _ADD_SHORT}    # 加仓
_CLOSE_POSITION_TYPES = {_CLOSE_LONG, _CLOSE_SHORT}  # 平仓
_LONG_TYPES = {_OPEN_LONG, _ADD_LONG, _CLOSE_LONG}
_SHORT_TYPES = {_OPEN_SHORT, _ADD_SHORT, _CLOSE_SHORT}
...
def rebuild_positions_from_fills(fills: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    Args:
        fills: 成交记录列表（需要包含 trade_type 字段，整数类型）
    ...
    for fill in sorted_fills:
        ...
        if trade_type is None or not fill_coin:
            continue
        
        volume = px * sz
        
        # 新开仓（从零仓位）
        if trade_type in _NEW_POSITION_TYPES:
            ...
            # 创建新仓位
            direction = 'long' if trade_type in _LONG_TYPES else 'short'
        ...
        # 加仓
        elif trade_type in _ADD_POSITION_TYPES:
        ...
        # 平仓
        elif trade_type in _CLOSE_POSITION_TYPES:
            if fill_coin not in positions_by_coin:
