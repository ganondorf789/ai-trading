...
class TraderMetricsOps:
    ...
    def _prepare_metrics_values(self, metrics: TraderMetrics) -> tuple:
        ...
        return (
            ...
            # 账户和保证金
            sanitize_value(self._get_metric_value(metrics, 'position.account_value', 0.0)),
            sanitize_value(self._get_metric_value(metrics, 'position.used_margin', 0.0)),
            sanitize_value(self._get_metric_value(metrics, 'position.perp_total_value', 0.0)),
            sanitize_value(self._get_metric_value(metrics, 'position.position_value', 0.0)),
            sanitize_value(self._get_metric_value(metrics, 'position.long_position_value', 0.0)),
            sanitize_value(self._get_metric_value(metrics, 'position.short_position_value', 0.0)),
            sanitize_value(self._get_metric_value(metrics, 'position.margin_usage_rate', 0.0)),
            # 多空分项
            sanitize_value(self._get_metric_value(metrics, 'trade.long_trades', 0), 0),
            sanitize_value(self._get_metric_value(metrics, 'trade.long_realized_pnl', 0.0)),
            sanitize_value(self._get_metric_value(metrics, 'trade.long_win_rate', 0.0)),
            sanitize_value(self._get_metric_value(metrics, 'trade.short_trades', 0), 0),
            sanitize_value(self._get_metric_value(metrics, 'trade.short_realized_pnl', 0.0)),
            sanitize_value(self._get_metric_value(metrics, 'trade.short_win_rate', 0.0)),
            sanitize_value(self._get_metric_value(metrics, 'position.long_position_ratio', 0.0)),
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                INSERT INTO trader_metrics (
                    ...
                    account_value, used_margin, perp_total_value, position_value,
                    long_position_value, short_position_value, margin_usage_rate,
                    long_trades, long_realized_pnl, long_win_rate,
                ...
                ON CONFLICT(address) DO UPDATE SET
                    ...
                    perp_total_value = EXCLUDED.perp_total_value,
                    ...
                    long_realized_pnl = EXCLUDED.long_realized_pnl,
                    long_win_rate = EXCLUDED.long_win_rate,
                    short_trades = EXCLUDED.short_trades,
                    short_realized_pnl = EXCLUDED.short_realized_pnl,
